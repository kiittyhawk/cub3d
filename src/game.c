...
/*   Updated: 2022/06/17 16:02:55 by jgyles           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	game_print(t_game *game)
{
	...
	ft_putstr_fd("X = ", 1);
	ft_putnbr_fd(game->ray.posX, 1);
	ft_putstr_fd(" , Y = ", 1);
	ft_putnbr_fd(game->ray.posY, 1);
...
t_game	*game_init(int fd)
{
	...
	game->mlx.addr = mlx_get_data_addr(game->mlx.img, &game->mlx.bpp, &game->mlx.line_l, &game->mlx.en);
	game->decor = read_decor(fd, game->mlx.mlx);
	if (!game->decor)
